...
from diffusion_reasoning_model import SudokuVAE, VAEConfig
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--steps", type=int, default=None, help="Deprecated alias for --max-steps")
...
def main() -> None:
    ...
    model = SudokuVAE(VAEConfig(d_model=args.d_model, d_z=args.d_z, layers=args.layers, heads=args.heads, beta=args.beta)).to(device)
    ...
    for step in progress:
        ...
        if step == 1 or step % max(1, args.max_steps // 10) == 0:
            ...
            tqdm.write(f"step={step} loss={out['loss'].item():.4f} recon={out['recon'].item():.4f} kl={out['kl'].item():.4f} exact={exact.item():.4f} cell={cell.item():.4f}")
