...
def sample_batch(model, puzzle_tokens: torch.Tensor, r_steps: int, k_samples: int) -> tuple[torch.Tensor, torch.Tensor]:
...
def main() -> None:
    ...
    model.eval()
    loader = make_loader(args.data_dir, args.split, args.batch_size, shuffle=False, limit=args.limit)
    ...
    print(
        f"n={n} exact={exact_sum/n:.4f} cell={cell_sum/n:.4f} "
        f"violations={violation_sum/n:.4f} q_mean={q_sum/n:.4f} R={args.R} K={args.K}"
    )
