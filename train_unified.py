...
from sudoku_diffusion.checkpoint import load_vae, save_unified, save_vae
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--run-dir", default=None, help="Defaults to checkpoints/sudoku/<TIMESTAMP>")
    ...
    p.add_argument("--train-limit", type=int, default=None, help="Group limit for group sampling; example limit for flat sampling")
    ...
    p.add_argument("--val-aug-count", type=int, default=0, help="Validation uses original plus this many augmented variants per held-out group")
    p.add_argument("--val-R", type=int, default=8, help="Rollout refinement steps for reasoner validation")
    p.add_argument("--val-K", type=int, default=1, help="Rollout samples per puzzle for reasoner validation")
    ...
    p.add_argument("--vae-patience-evals", type=int, default=0, help="0 disables patience early stopping")
    p.add_argument("--vae-min-aug-epochs", type=float, default=1.0, help="Minimum full augmented train-set passes before VAE early stopping can trigger")
...
def configure_output_paths(args: argparse.Namespace) -> None:
    run_dir = Path(args.run_dir) if args.run_dir is not None else Path("checkpoints") / "sudoku" / datetime.now().strftime("%Y%m%d_%H%M%S")
...
def set_warmup_lr(opt: torch.optim.Optimizer, base_lr: float, step: int, warmup_steps: int) -> None:
    scale = min(1.0, step / max(1, warmup_steps)) if warmup_steps > 0 else 1.0
    for group in opt.param_groups:
        group["lr"] = base_lr * scale


def augmented_train_examples(data_dir: str, train_group_count: int, train_limit: int | None) -> int:
    if train_limit is not None:
        ...
        for group_id in range(min(train_group_count, arrays.total_groups)):
            total += int(arrays.group_indices[group_id + 1] - arrays.group_indices[group_id])
        return max(total, 1)
    with (Path(data_dir) / "train" / "dataset.json").open("r") as f:
        metadata = json.load(f)
    total_groups = max(int(metadata["total_groups"]), 1)
    total_examples = int(round(float(metadata["total_puzzles"]) * float(metadata["mean_puzzle_examples"])))
    return max(1, math.ceil(total_examples * train_group_count / total_groups))

def flat_batches(data_dir: str, batch_size: int, total_steps: int, train_limit: int | None):
...
def evaluate_vae(model: SudokuVAE, args: argparse.Namespace, device: torch.device, val_group_ids) -> dict[str, float]:
    was_training = model.training
    model.eval()
    total = 0
    sums = {"loss": 0.0, "recon": 0.0, "kl": 0.0, "exact": 0.0, "cell": 0.0}
    for raw in iter_group_eval_batches(args.data_dir, args.eval_batch_size, val_group_ids, aug_count=args.val_aug_count):
...
@torch.inference_mode()
def evaluate_reasoner(model: UnifiedLatentReasoner, args: argparse.Namespace, device: torch.device, val_group_ids) -> dict[str, float]:
    ...
    for raw in iter_group_eval_batches(args.data_dir, args.eval_batch_size, val_group_ids, aug_count=args.val_aug_count):
        batch = to_device(raw, device)
        pred_digits, q_logits = model.sample(batch.puzzle_tokens, r_steps=args.val_R, k_samples=args.val_K)
...
def train_vae(args: argparse.Namespace, device: torch.device, train_group_ids, val_group_ids) -> SudokuVAE:
    model = SudokuVAE(
        VAEConfig(
    ...
    min_aug_examples = augmented_train_examples(args.data_dir, train_group_count, args.train_limit if args.sampling == "group" else None)
    min_early_stop_steps = math.ceil(args.vae_min_aug_epochs * min_aug_examples / args.vae_batch_size)
    epochs_needed = math.ceil(args.vae_max_steps * args.vae_batch_size / train_group_count) + 1
    eval_interval_steps = max(1, math.ceil(args.eval_every_epochs * train_group_count / args.vae_batch_size))
    group_iter = iter_group_batches(args.data_dir, args.vae_batch_size, train_group_ids, epochs_needed, seed=args.seed)
    flat_iter = flat_batches(args.data_dir, args.vae_batch_size, args.vae_max_steps, args.train_limit)
    print(f"stage=vae max_steps={args.vae_max_steps} sampling={args.sampling} eval_every_epochs={args.eval_every_epochs} min_aug_examples={min_aug_examples} min_early_stop_steps={min_early_stop_steps}")
    progress = tqdm(range(1, args.vae_max_steps + 1), desc="vae", total=args.vae_max_steps)
    for step in progress:
        ...
        if step == 1 or step % 100 == 0:
            pred = out["logits"].argmax(dim=-1) + 1
            exact, cell = exact_and_cell_accuracy(pred, batch.solution_digits)
            progress.set_postfix(epoch=f"{epoch_done:.1f}", loss=f"{out['loss'].item():.4f}", exact=f"{exact.item():.4f}", cell=f"{cell.item():.4f}")
            tqdm.write(f"stage=vae step={step} epoch={epoch_done:.2f} loss={out['loss'].item():.4f} recon={out['recon'].item():.4f} kl={out['kl'].item():.4f} exact={exact.item():.4f} cell={cell.item():.4f}")
        if val_group_ids.size and (step == 1 or step % eval_interval_steps == 0 or step == args.vae_max_steps):
            metrics = evaluate_vae(model, args, device, val_group_ids)
            tqdm.write(f"stage=vae_eval step={step} epoch={epoch_done:.2f} val_loss={metrics['loss']:.4f} val_recon={metrics['recon']:.4f} val_exact={metrics['exact']:.4f} val_cell={metrics['cell']:.4f}")
            is_better = (metrics["exact"] > best_val_exact) or (metrics["exact"] == best_val_exact and metrics["recon"] < best_val_recon)
            if is_better:
                best_val_exact = metrics["exact"]
                best_val_recon = metrics["recon"]
                stale_evals = 0
                if best_vae_out is not None:
                    save_vae(best_vae_out, model, {"steps": step, "selection": "max_exact_then_min_recon", "val_metrics": metrics})
                    tqdm.write(f"saved best VAE {best_vae_out}")
            else:
                stale_evals += 1
            early_stop_allowed = step >= min_early_stop_steps
            if not early_stop_allowed and (args.vae_target_val_exact is not None or args.vae_patience_evals > 0):
                tqdm.write(f"VAE early stop disabled until step {min_early_stop_steps} ({args.vae_min_aug_epochs:g} augmented epoch floor)")
            if early_stop_allowed and args.vae_target_val_exact is not None and metrics["exact"] >= args.vae_target_val_exact:
                tqdm.write(f"early stop VAE: val_exact {metrics['exact']:.4f} >= target {args.vae_target_val_exact:.4f}")
                break
            if early_stop_allowed and args.vae_patience_evals > 0 and stale_evals >= args.vae_patience_evals:
                tqdm.write(f"early stop VAE: no validation improvement for {stale_evals} evals")
                break
    return model


@torch.inference_mode()
def compute_latent_stats(args: argparse.Namespace, device: torch.device, vae: SudokuVAE, train_group_ids) -> tuple[torch.Tensor, torch.Tensor, int]:
    vae.eval()
    count = 0
    total = None
    total_sq = None
    print("stage=stats")
    for raw in tqdm(iter_group_all_example_batches(args.data_dir, args.stats_batch_size, train_group_ids), desc="stats"):
        batch = to_device(raw, device)
    ...
    std = var.sqrt()
    print(f"stage=stats count={count} mean_abs={mean.abs().mean().item():.4f} std_mean={std.mean().item():.4f}")
    return mean, std, count


def train_reasoner(args: argparse.Namespace, device: torch.device, model: UnifiedLatentReasoner, train_group_ids, val_group_ids) -> int:
    ...
    train_group_count = max(len(train_group_ids), 1)
    total_steps = args.reasoner_steps if args.reasoner_steps is not None else math.ceil(args.reasoner_epochs * train_group_count / args.reasoner_batch_size)
    epochs_needed = math.ceil(total_steps * args.reasoner_batch_size / train_group_count) + 1
    eval_interval_steps = max(1, math.ceil(args.eval_every_epochs * train_group_count / args.reasoner_batch_size))
    group_iter = iter_group_batches(args.data_dir, args.reasoner_batch_size, train_group_ids, epochs_needed, seed=args.seed + 100000)
    flat_iter = flat_batches(args.data_dir, args.reasoner_batch_size, total_steps, args.train_limit)
    best_val_exact = -1.0
    best_val_loss = float("inf")
    best_unified_out = best_path(args.out, args.best_out)
    print(f"stage=reasoner total_steps={total_steps} epochs={args.reasoner_epochs} sampling={args.sampling} train_groups={train_group_count}")
    ...
    for step in progress:
        ...
        if step == 1 or step % 100 == 0:
            exact, cell = exact_and_cell_accuracy(pred_digits, batch.solution_digits)
            progress.set_postfix(epoch=f"{epoch_done:.1f}/{args.reasoner_epochs}", loss=f"{loss.item():.4f}", exact=f"{exact.item():.4f}", cell=f"{cell.item():.4f}")
            tqdm.write(f"stage=reasoner step={step} epoch={epoch_done:.2f}/{args.reasoner_epochs} loss={loss.item():.4f} fm={fm_loss.item():.4f} ce={answer_loss.item():.4f} q={q_loss.item():.4f} exact={exact.item():.4f} cell={cell.item():.4f}")
        if val_group_ids.size and (step == 1 or step % eval_interval_steps == 0 or step == total_steps):
            metrics = evaluate_reasoner(model, args, device, val_group_ids)
            tqdm.write(f"stage=reasoner_eval_rollout step={step} epoch={epoch_done:.2f} R={args.val_R} K={args.val_K} val_exact={metrics['exact']:.4f} val_cell={metrics['cell']:.4f} val_q={metrics['q']:.4f}")
            is_better = (metrics["exact"] > best_val_exact) or (metrics["exact"] == best_val_exact and metrics["loss"] < best_val_loss)
            if is_better:
                best_val_exact = metrics["exact"]
                best_val_loss = metrics["loss"]
                save_unified(best_unified_out, model, {"reasoner_steps": step, "reasoner_epoch": epoch_done, "selection": "max_rollout_exact", "val_metrics": metrics})
...
def main() -> None:
    ...
    train_group_ids, val_group_ids = split_group_ids(args.data_dir, args.val_group_fraction, seed=args.seed, limit_groups=args.train_limit if args.sampling == "group" else None)
    print(f"groups train={train_group_ids.size} val={val_group_ids.size} sampling={args.sampling}")
    ...
    reasoner_steps = train_reasoner(args, device, unified, train_group_ids, val_group_ids)
    save_unified(
        args.out,
        unified,
        {
